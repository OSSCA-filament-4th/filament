...
TEST_F(glTFIOTest, AnimatedMorphCubeTransforms) {
    ...
    auto inst = transformManager.getInstance(renderables[0]);
    math::mat4f transform = transformManager.getTransform(inst);
    math::mat4f expectedTransform = composeMatrix(math::float3{0.0, 0.0, 0.0},
...
TEST_F(glTFIOTest, AnimatedMorphCubeRenderables) {
    ...
    AttributeBitset attribs = renderableManager.getEnabledAttributesAt(inst, 0);
    ...
    auto morphTargetBuffer = renderableManager.getMorphTargetBufferAt(inst, 0, 0);
